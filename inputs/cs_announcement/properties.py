...
class Properties:
    ...
    def compute_constraints(self, TS, setup_helper, removed_transitions):
        ...
        self.get_counterexample(5,2*n+1,f_T,f_M,n,self.no_further_announcement_constraint,setup_helper,setup_constraints,results,counterexamples, "no further")
        self.get_counterexample(6,2*n+1,f_T,f_M,n,self.announcement_exists_constraint,setup_helper,setup_constraints,results,counterexamples, "announcement")
    ...
    def no_further_announcement_constraint(self, f_T, f_M, n, inputs, outputs, micros, inps, sts):
        ...
        for i in range(2*n-1):
            curr_state_constraint = And(sts[i]!=-1, f_M(sts[i])==micros["Announcement"], inps[i]==inputs["Ready"])

            to_negate_constraint = Or(False)
            for j in range(i+1,2*n-1):
                to_negate_constraint = Or(to_negate_constraint, And(sts[j]!=-1, f_M(sts[j])==micros["Announcement"]))
    ...
    def announcement_exists_constraint(self, f_T, f_M, n, inputs, outputs, micros, inps, sts):
        ...
        for i in range(2*n-1):
            #negation_constraint = Or(negation_constraint, And(sts[i]>=0, f_M(sts[i])==outputs["Farewell"], f_T(sts[i], inputs["Ready"])==-1, f_T(sts[i], inputs["Ignore"])==-1))
            negation_constraint = Or(negation_constraint, And(sts[i]>=0,f_M(sts[i])==micros["Announcement"]))
