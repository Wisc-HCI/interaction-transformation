...
def array_representation(fsm, tlacenode,spec):
    global __id_node
    __id_node = 0
    array_rep = []
    array_node(fsm,tlacenode,array_rep)
    #for branch in tlacenode.branches:
    #    array_branch(fsm, branch, array_rep)
    #print(array_rep)
    #print(xml_representation(fsm,tlacenode,spec))
    return array_rep

def array_node(fsm, node, array_rep):

    if len(node.branches) == 0:
        array_rep.append(array_state(fsm, node.state))

    for branch in node.branches:
        array_branch(fsm, branch, array_rep)
        break  # ensure we only receive one counterexample

def array_branch(fsm, branch, array_rep):
    for n, i in zip(branch.path[0][::2], branch.path[0][1::2]):
        array_node(fsm, n, array_rep)
    array_node(fsm, branch.path[0][-1], array_rep)

def array_state(fsm, state):
    counter_step = {}
    values = state.get_str_values()
    for var in values:
        counter_step[var] = values[var]
    return counter_step
...
def get_counterexample(fsm,state):
    counterexample = []

    values = state.get_str_values()
    for var in values:
        counterexample.append({"{}: {}".format(var,values[var])})
